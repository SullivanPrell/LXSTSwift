...
final class ParityGapTests: XCTestCase {
  ...
  func testCodec2DecodeAdoptsWireModeHeaderDifferentBPF() throws {
    ...
    XCTAssertEqual(
      wire[wire.startIndex], Codec2Mode.mode3200.headerByte,
    ...
    native.sink = sink
    XCTAssertEqual(
      decoded.sampleCount, try native.decode(wire).sampleCount,
  ...
  func testOpusFileSinkFinalizeTimeout() {
    XCTAssertEqual(
      OpusFileSink.finalizeTimeout, 2,
  ...
  private final class MockCodec: Codec {
    ...
    func decode(_ data: Data) throws -> AudioFrame {
      AudioFrame(samples: [], channelCount: 1, sampleRate: 48000)
    }
  }
}
