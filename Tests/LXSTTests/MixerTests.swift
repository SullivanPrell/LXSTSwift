...
final class MixerTests: XCTestCase {
  ...
  final class MockReferenceSink: ReferenceSink {
    ...
    func handleReference(_ frame: AudioFrame, samplerate: Double) {
      lock.lock()
      unsafeFrames.append(frame)
      unsafeRates.append(samplerate)
      lock.unlock()
      onReceive?()
    }
  }
  ...
  func testConcurrentControlPlaneStressIsRaceFree() {
    ...
    let frame = AudioFrame(
      samples: [Float](repeating: 0.25, count: n),
    ...
    spawn {
      ...
      var i = 0
      while Date() < deadline {
        m.referenceOuts = (i % 2 == 0) ? refs : []
        i += 1
      }
    }
    // Producer: frames into the queue + backpressure check
    spawn {
      while Date() < deadline { if m.canReceive(from: src) { m.handleFrame(frame, from: src) } }
    }
    ...
    spawn {
      var k = 1
      while Date() < deadline {
        m.setSourceMaxFrames((k % 8) + 1, for: src)
        k += 1
      }
    }
    ...
    m.start()
    XCTAssertTrue(m.shouldRun, "start() must set the run flag through the lock")
    m.stop()
    XCTAssertFalse(m.shouldRun, "stop() must clear the run flag through the lock")
  }
  ...
  func testStopRacingControlWritesIsRaceFree() {
    ...
    let frame = AudioFrame(
      samples: [Float](repeating: 0.1, count: n),
      channelCount: 1, sampleRate: 48000)
  ...
  func testReferenceOutReceivesMixedFrame() {
    ...
    m.handleFrame(
      AudioFrame(
        samples: [Float](repeating: 0.5, count: n),
