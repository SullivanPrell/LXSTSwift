...
final class EchoSuppressorTests: XCTestCase {
  ...
  func testDecimateMatchesReference() {
    ...
    let expY1: [Float] = [
      0.0, 0.010267061181366444, 0.0650000050663948, 0.1249999925494194,
      0.1850000023841858, 0.24500000476837158, 0.3049999475479126,
      0.36500000953674316, 0.42499998211860657, 0.48500001430511475,
    ]
    let expY2: [Float] = [
      0.5450000166893005, 0.6049999594688416, 0.6649999618530273,
  ...
  private func lcgStream(count: Int, seed: Int) -> [Float] {
    ...
    let mask = 0x7fff_ffff
    for i in 0..<count {
      state = (state &* 1_103_515_245 &+ 12345) & mask
  ...
  func testPureEchoIsDetectedAndGated() {
    ...
    for f in 0..<totalFrames {
      ...
      es.handleReference(
        AudioFrame(samples: refFrame, channelCount: 1, sampleRate: sr),
        samplerate: sr)
    ...
    XCTAssertEqual(
      es.delaySamples ?? 0, Double(trueDelay), accuracy: 60,
