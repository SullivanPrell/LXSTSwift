...
final class PowerDBGainParityTests: XCTestCase {
  ...
  func testLineSourceDeliverAppliesPowerDBGainToMicSamples() {
    ...
    XCTAssertEqual(
      received[0].samples[0], 0.5, accuracy: 1e-4,
  ...
  func testMixerMixLoopAppliesPowerDBMixingGain() {
    ...
    m.handleFrame(
      AudioFrame(
        samples: [Float](repeating: 0.05, count: n),
    ...
    XCTAssertEqual(
      sink.frames.first?.samples.first ?? .nan, 0.005, accuracy: 1e-4,
  ...
  func testAGCNormalizesToPowerDBTargetLevel() {
    ...
    XCTAssertEqual(
      out.samples[0], 0.0316228, accuracy: 0.001,
  ...
  func testAGCMaxGainUsesPowerDBConvention() {
    ...
    let frame = AudioFrame(
      samples: [Float](repeating: 0.0035, count: 480),
      channelCount: 1, sampleRate: 48000)
    var out = frame
    for _ in 0..<200 { out = agc.handleFrame(frame) }
    ...
    XCTAssertEqual(
      out.samples[0], 0.0554712, accuracy: 0.001,
