...
final class TelephonyTests: XCTestCase {
  ...
  func testSignallingPreferredProfile() {
    XCTAssertEqual(
      signallingPreferredProfile, 0xFF,
  ...
  func testTelephoneDialToneFrequency() {
    XCTAssertEqual(
      Telephone.dialToneFrequency, 382.0, accuracy: 0.001,
      "DIAL_TONE_FREQUENCY must be 382 Hz")
  }
  func testTelephoneDialToneEaseMs() {
    XCTAssertEqual(
      Telephone.dialToneEaseMs, 3.14159, accuracy: 0.00001,
  ...
  func testTelephoneServesLxstTelephonyDestination() throws {
    ...
    XCTAssertEqual(
      phone.destination?.hash, expected.hash,
  ...
  func testSetAnnounceIntervalClampsToMin() {
    ...
    XCTAssertGreaterThanOrEqual(
      phone.announceIntervalSetting, Telephone.announceIntervalMin,
  ...
  func testHangupBusyFiresBusyCallback() {
    ...
    XCTAssertFalse(
      endedCalled, "hangup(reason:.busy) must NOT fire ended callback if busy callback set")
  }
  ...
  func testNextProfileWrapsAround() {
    XCTAssertEqual(
      TelephonyProfile.next(after: TelephonyProfile.available.last!),
