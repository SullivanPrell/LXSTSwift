...
public final class LineSink: LocalSink {
  ...
  private func adoptDeviceChannelMapIfChanged() {
    ...
    Reticulum.log(
      "Underlying device for LineSink re-configured channel map mid-stream (from \(channels.map(String.init) ?? "nil") to \(deviceChannels))",
      level: .warning)
    ...
    if let replacement = try? backend.startPlayback(
      ...
      channelCount: deviceChannels)
    {
      ...
      Reticulum.log(
        "Could not restart playback at \(deviceChannels) channels; falling back to \(previousChannels.map(String.init) ?? "1")",
        level: .warning)
      player = try? backend.startPlayback(
        sampleRate: sampleRate,
