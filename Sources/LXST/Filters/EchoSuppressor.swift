...
public final class EchoSuppressor: Filter, ReferenceSink {
  ...
  public init(
    maxDelayMs: Double = defaultMaxDelayMs,
    ...
    self.decimTaps = EchoSuppressor.designDecimatorTaps(
      numTaps: 12, cutoffHz: 3700, samplerate: 48000)
  ...
  private func appendCircular(
    _ buffer: inout [Float], _ writePos: inout Int, _ valid: inout Int,
    _ size: Int, _ mono: [Float]
  ) {
    var m = mono
    var count = m.count
    if count > size {
      m = Array(m.suffix(size))
      count = size
    }
  ...
  private func readCircular(
    _ buffer: [Float], _ writePos: Int, _ size: Int,
    length rawLength: Int, valid: Int
  ) -> [Float] {
  ...
  private func getMicHistoryDs() -> [Float] {
    readCircular(
      micHistDs, micHistWriteDs, correlationSamplesDs,
  ...
  private func updateCoupling(micEnergy: Double, refEnergy: Double, corr: Double) {
    ...
    if refEnergy > refEnergyThreshold && corr > corrThreshold && micEnergy > micEnergyThreshold {
      ...
      if couplingHistory.count >= EchoSuppressor.requiredCouplingHistory {
        ...
        if couplingDbValue < couplingThresholdDb {
          disabledByCoupling = true
        } else if couplingDbValue > couplingThresholdDb + 1.5 {
          disabledByCoupling = false
        }
      }
    }
  ...
  public func handleFrame(_ frame: AudioFrame) -> AudioFrame {
    ...
    if currentGain < 1e-6 { currentGain = 0.0 } else if currentGain > 1.0 { currentGain = 1.0 }
  ...
  private func estimateDelay(refWindow: [Float], micWindow: [Float]) {
    ...
    for i in 0..<refCount {
      cumsum[i + 1] = cumsum[i] + Double(refWindow[i]) * Double(refWindow[i])
    }
    ...
    if let ds = delaySamplesValue {
      ...
      if dMax >= dMin {
        for i in dMin...dMax where accNorm[i] > localVal {
          localVal = accNorm[i]
          localBest = i
        }
      }
  ...
  private func argmax(_ x: [Double]) -> (Int, Double) {
    ...
    for i in 1..<x.count where x[i] > bv {
      bv = x[i]
      bi = i
    }
    return (bi, bv)
