...
public final class RawCodec: Codec {
  ...
  public static func subHeader(bitDepth: Int, channelCount: Int) -> UInt8 {
    let depthBits: UInt8
    if bitDepth >= 128 {
      depthBits = RawBitDepth.float128.rawValue
    } else if bitDepth >= 64 {
      depthBits = RawBitDepth.float64.rawValue
    } else if bitDepth >= 32 {
      depthBits = RawBitDepth.float32.rawValue
    } else {
      depthBits = RawBitDepth.float16.rawValue
    }
